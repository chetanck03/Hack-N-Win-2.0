...
contract Vote {
    ...
    // Function to change Election Commission address
    function changeElectionComission(address _newComission) external onlyComissioner {
        require(_newComission != address(0), "Invalid address");
        electionComission = _newComission;
    }
    ...
    function resetVotingSystem() external onlyComissioner {
        for (uint i = 1; i < nextVoterId; i++) {
    ...
    // Function to reset voters only
    function resetVoters() external onlyComissioner {
        for (uint i = 1; i < nextVoterId; i++) {
            delete voterDetails[i];
        }
        nextVoterId = 1;
    ...
    // Function to reset candidates only
    function resetCandidates() external onlyComissioner {
        for (uint i = 1; i < nextCandidateId; i++) {
            delete candidateDetails[i];
        }
        nextCandidateId = 1;
    ...
    // Functions to handle Ether transactions

    receive() external payable {
    }

    function withdrawFunds(address payable _to, uint _amount) external onlyComissioner {
